...
class AttendanceBase(models.Model):
    ...
    roll_number = models.ForeignKey(Phase3_P2Student, on_delete=models.CASCADE)
...
class Psychiatry(AttendanceBase):
    class Meta:
        unique_together = ('roll_number', 'date')


class Dermatology(AttendanceBase):
    class Meta:
        unique_together = ('roll_number', 'date')


class Radiology(AttendanceBase):
    class Meta:
        unique_together = ('roll_number', 'date')
...
class Anaesthsiology(AttendanceBase):
    class Meta:
        unique_together = ('roll_number', 'date')
